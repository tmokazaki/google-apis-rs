...
use google_cloudsupport2_beta::{api, yup_oauth2, Error};
...
{
    async fn _case_classifications_search(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(
                                    ["page-size", "page-token", "product-product-line", "query"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _cases_attachments_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .cases()
            .attachments_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _cases_close(
        ...
        let mut request: api::CloseCaseRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .cases()
            .close(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _cases_comments_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "creator.google-support" => Some((
                    "creator.googleSupport",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "creator.username" => Some((
                    "creator.username",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
        ...
        let mut call = self
            .hub
            .cases()
            .comments_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _cases_comments_list(
        ...
        let mut call = self
            .hub
            .cases()
            .comments_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _cases_create(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "classification.display-name" => Some((
                ...
                "classification.product.product-line" => Some((
                    "classification.product.productLine",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                ...
                "subscriber-email-addresses" => Some((
        ...
        let mut request: api::Case = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .cases()
    ...
    async fn _cases_escalate(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["escalation", "justification", "reason"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::EscalateCaseRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .cases()
            .escalate(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["filter", "page-size", "page-token", "product-line"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["page-size", "page-token", "parent", "query"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _cases_show_feed(
        ...
        let mut call = self
            .hub
            .cases()
            .show_feed(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _media_upload(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "attachment.creator.display-name" => Some((
                ...
                "attachment.creator.google-support" => Some((
                    "attachment.creator.googleSupport",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "attachment.creator.username" => Some((
                    "attachment.creator.username",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::CreateAttachmentRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .media()
            .upload(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("case-classifications", Some(opt)) => match opt.subcommand() {
                ("search", Some(opt)) => {
                    call_result = self
                        ._case_classifications_search(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "case-classifications".to_string(),
                    ));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("cases", Some(opt)) => match opt.subcommand() {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("cases".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudsupport2-beta-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/cloudsupport2-beta", config_dir))
        .build()
        .await
        .unwrap();
