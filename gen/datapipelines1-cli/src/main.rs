...
use google_datapipelines1::{api, yup_oauth2, Error};
...
{
    async fn _projects_locations_pipelines_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::GoogleCloudDatapipelinesV1Pipeline =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_jobs_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_jobs_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_run(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::GoogleCloudDatapipelinesV1RunPipelineRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_run(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_pipelines_stop(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::GoogleCloudDatapipelinesV1StopPipelineRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_pipelines_stop(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("locations-pipelines-create", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_create(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-delete", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-get", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-jobs-list", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_jobs_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-list", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-patch", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-run", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_run(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-pipelines-stop", Some(opt)) => {
                    call_result = self
                        ._projects_locations_pipelines_stop(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "datapipelines1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/datapipelines1", config_dir))
        .build()
        .await
        .unwrap();
