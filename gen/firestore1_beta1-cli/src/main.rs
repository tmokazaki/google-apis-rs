...
use google_firestore1_beta1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _projects_databases_export_documents(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["collection-ids", "output-uri-prefix"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::GoogleFirestoreAdminV1beta1ExportDocumentsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
    ...
    async fn _projects_databases_import_documents(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["collection-ids", "input-uri-prefix"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::GoogleFirestoreAdminV1beta1ImportDocumentsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
    ...
    async fn _projects_databases_indexes_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["collection-id", "name", "state"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::GoogleFirestoreAdminV1beta1Index =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .databases_indexes_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_databases_indexes_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .databases_indexes_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_databases_indexes_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .databases_indexes_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_databases_indexes_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .databases_indexes_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ...
                ("databases-indexes-create", Some(opt)) => {
                    call_result = self
                        ._projects_databases_indexes_create(opt, dry_run, &mut err)
                        .await;
                }
                ("databases-indexes-delete", Some(opt)) => {
                    call_result = self
                        ._projects_databases_indexes_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("databases-indexes-get", Some(opt)) => {
                    call_result = self
                        ._projects_databases_indexes_get(opt, dry_run, &mut err)
                        .await;
                }
                ("databases-indexes-list", Some(opt)) => {
                    call_result = self
                        ._projects_databases_indexes_list(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "firestore1-beta1-secret.json",
