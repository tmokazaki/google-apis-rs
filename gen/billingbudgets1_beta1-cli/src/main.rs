...
use google_billingbudgets1_beta1::{api, yup_oauth2, Error};
...
{
    async fn _billing_accounts_budgets_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            .iter()
        {
            ...
            {
                "budget.all-updates-rule.disable-default-iam-recipients" => Some((
                ...
                "budget.all-updates-rule.enable-project-level-recipients" => Some((
                    "budget.allUpdatesRule.enableProjectLevelRecipients",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.all-updates-rule.monitoring-notification-channels" => Some((
                    "budget.allUpdatesRule.monitoringNotificationChannels",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "budget.all-updates-rule.pubsub-topic" => Some((
                    "budget.allUpdatesRule.pubsubTopic",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.all-updates-rule.schema-version" => Some((
                    "budget.allUpdatesRule.schemaVersion",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.amount.specified-amount.currency-code" => Some((
                ...
                "budget.amount.specified-amount.nanos" => Some((
                    "budget.amount.specifiedAmount.nanos",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.amount.specified-amount.units" => Some((
                ...
                "budget.budget-filter.calendar-period" => Some((
                ...
                "budget.budget-filter.credit-types" => Some((
                    "budget.budgetFilter.creditTypes",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "budget.budget-filter.credit-types-treatment" => Some((
                ...
                "budget.budget-filter.custom-period.end-date.day" => Some((
                    "budget.budgetFilter.customPeriod.endDate.day",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.budget-filter.custom-period.end-date.month" => Some((
                    "budget.budgetFilter.customPeriod.endDate.month",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.budget-filter.custom-period.end-date.year" => Some((
                ...
                "budget.budget-filter.custom-period.start-date.day" => Some((
                    "budget.budgetFilter.customPeriod.startDate.day",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.budget-filter.custom-period.start-date.month" => Some((
                    "budget.budgetFilter.customPeriod.startDate.month",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "budget.budget-filter.custom-period.start-date.year" => Some((
                ...
                "budget.budget-filter.projects" => Some((
                ...
                "budget.budget-filter.resource-ancestors" => Some((
                    "budget.budgetFilter.resourceAncestors",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "budget.budget-filter.services" => Some((
                ...
                "budget.budget-filter.subaccounts" => Some((
                    "budget.budgetFilter.subaccounts",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "budget.display-name" => Some((
                    "budget.displayName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
        ...
        let mut request: api::GoogleCloudBillingBudgetsV1beta1CreateBudgetRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .billing_accounts()
            .budgets_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _billing_accounts_budgets_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .billing_accounts()
            .budgets_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _billing_accounts_budgets_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .billing_accounts()
            .budgets_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _billing_accounts_budgets_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .billing_accounts()
            .budgets_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _billing_accounts_budgets_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::GoogleCloudBillingBudgetsV1beta1UpdateBudgetRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .billing_accounts()
            .budgets_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("billing-accounts", Some(opt)) => match opt.subcommand() {
                ("budgets-create", Some(opt)) => {
                    call_result = self
                        ._billing_accounts_budgets_create(opt, dry_run, &mut err)
                        .await;
                }
                ("budgets-delete", Some(opt)) => {
                    call_result = self
                        ._billing_accounts_budgets_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("budgets-get", Some(opt)) => {
                    call_result = self
                        ._billing_accounts_budgets_get(opt, dry_run, &mut err)
                        .await;
                }
                ("budgets-list", Some(opt)) => {
                    call_result = self
                        ._billing_accounts_budgets_list(opt, dry_run, &mut err)
                        .await;
                }
                ("budgets-patch", Some(opt)) => {
                    call_result = self
                        ._billing_accounts_budgets_patch(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "billingbudgets1-beta1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/billingbudgets1-beta1", config_dir))
        .build()
        .await
        .unwrap();
