...
use google_tpu1_alpha1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                let mut v = Vec::new();
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(["node-id", "request-id"].iter().map(|v| *v));
                                v
                            }));
                    }
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "tpu1-alpha1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/tpu1-alpha1", config_dir))
        .build()
        .await
        .unwrap();
