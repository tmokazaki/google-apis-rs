...
use google_youtubereporting1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _jobs_create(
        ...
        let mut call = self.hub.jobs().create(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _jobs_delete(
        ...
        let mut call = self.hub.jobs().delete(opt.value_of("job-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _jobs_list(
        ...
        let mut call = self.hub.jobs().list();
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                ...
                "include-system-managed" => {
                    call = call.include_system_managed(
                        value
                            .map(|v| arg_from_str(v, err, "include-system-managed", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
    ...
    async fn _jobs_reports_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.jobs().reports_get(
    ...
    async fn _jobs_reports_list(
        ...
        let mut call = self
            .hub
            .jobs()
            .reports_list(opt.value_of("job-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                "start-time-before" => {
                    call = call.start_time_before(
                        value
                            .map(|v| arg_from_str(v, err, "start-time-before", "google-datetime"))
                            .unwrap_or(chrono::Utc::now()),
                    );
                }
                "start-time-at-or-after" => {
                    call = call.start_time_at_or_after(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "start-time-at-or-after", "google-datetime")
                            })
                            .unwrap_or(chrono::Utc::now()),
                    );
                }
                ...
                "created-after" => {
                    call = call.created_after(
                        value
                            .map(|v| arg_from_str(v, err, "created-after", "google-datetime"))
                            .unwrap_or(chrono::Utc::now()),
                    );
                }
                _ => {
    ...
    async fn _media_download(
        ...
        let mut call = self
            .hub
            .media()
            .download(opt.value_of("resource-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _report_types_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.report_types().list();
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("jobs", Some(opt)) => match opt.subcommand() {
                ("create", Some(opt)) => {
                    call_result = self._jobs_create(opt, dry_run, &mut err).await;
                }
                ("delete", Some(opt)) => {
                ...
                ("reports-get", Some(opt)) => {
                    call_result = self._jobs_reports_get(opt, dry_run, &mut err).await;
                }
                ("reports-list", Some(opt)) => {
                    call_result = self._jobs_reports_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
            ...
            ("report-types", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._report_types_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("report-types".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "youtubereporting1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/youtubereporting1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("jobs", "methods: 'create', 'delete', 'get', 'list', 'reports-get' and 'reports-list'", vec![
            ("create",
                    Some(r##"Creates a job and returns it."##),
                    "Details at http://byron.github.io/google-apis-rs/google_youtubereporting1_cli/jobs_create",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("delete",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_youtubereporting1_cli/jobs_delete",
                  vec![
                    (Some(r##"job-id"##),
                     None,
                     Some(r##"The ID of the job to delete."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("get",
                    Some(r##"Gets a job."##),
                    "Details at http://byron.github.io/google-apis-rs/google_youtubereporting1_cli/jobs_get",
                  vec![
                    (Some(r##"job-id"##),
                     None,
                     Some(r##"The ID of the job to retrieve."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("list",
                    Some(r##"Lists jobs."##),
                    "Details at http://byron.github.io/google-apis-rs/google_youtubereporting1_cli/jobs_list",
                  vec![
                    (Some(r##"v"##),
                     Some(r##"p"##),
                     Some(r##"Set various optional parameters, matching the key=value form"##),
            ...
            ("reports-get",
                    Some(r##"Gets the metadata of a specific report."##),
                    "Details at http://byron.github.io/google-apis-rs/google_youtubereporting1_cli/jobs_reports-get",
                  vec![
                    (Some(r##"job-id"##),
                     None,
                     Some(r##"The ID of the job."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"report-id"##),
                     None,
                     Some(r##"The ID of the report to retrieve."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("report-types", "methods: 'list'", vec![
            ("list",
                    Some(r##"Lists report types."##),
                    "Details at http://byron.github.io/google-apis-rs/google_youtubereporting1_cli/report-types_list",
                  vec![
                    (Some(r##"v"##),
                     Some(r##"p"##),
                     Some(r##"Set various optional parameters, matching the key=value form"##),
