...
use google_drive3::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _changes_list(
        ...
        let mut call = self
            .hub
            .changes()
            .list(opt.value_of("page-token").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                ...
                "restrict-to-my-drive" => {
                    call = call.restrict_to_my_drive(
                        value
                            .map(|v| arg_from_str(v, err, "restrict-to-my-drive", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "page-size" => {
                ...
                "include-removed" => {
                    call = call.include_removed(
                        value
                            .map(|v| arg_from_str(v, err, "include-removed", "boolean"))
                            .unwrap_or(false),
                    );
                }
    ...
    async fn _changes_watch(
        ...
        let mut call = self
            .hub
            .changes()
            .watch(request, opt.value_of("page-token").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _comments_create(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "quoted-file-content.mime-type" => Some((
                ...
                "quoted-file-content.value" => Some((
        ...
        let mut call = self
            .hub
            .comments()
            .create(request, opt.value_of("file-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _drives_create(
        ...
        let mut call = self
            .hub
            .drives()
            .create(request, opt.value_of("request-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["page-size", "page-token", "q", "use-domain-admin-access"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _files_copy(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "capabilities.can-change-viewers-can-copy-content" => Some((
                ...
                "content-hints.indexable-text" => Some((
                ...
                "content-hints.thumbnail.image" => Some((
                ...
                "content-hints.thumbnail.mime-type" => Some((
                ...
                "image-media-metadata.time" => Some((
                ...
                "last-modifying-user.photo-link" => Some((
            ...
            .iter()
        {
            ...
            match key {
                ...
                "keep-revision-forever" => {
                    call = call.keep_revision_forever(
                        value
                            .map(|v| arg_from_str(v, err, "keep-revision-forever", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "include-permissions-for-view" => {
                ...
                "ignore-default-visibility" => {
                    call = call.ignore_default_visibility(
                        value
                            .map(|v| arg_from_str(v, err, "ignore-default-visibility", "boolean"))
                            .unwrap_or(false),
                    );
                }
            ...
            .iter()
        {
            ...
            match key {
                ...
                "send-notification-email" => {
                    call = call.send_notification_email(
                        value
                            .map(|v| arg_from_str(v, err, "send-notification-email", "boolean"))
                            .unwrap_or(false),
                    );
                }
    ...
    async fn _replies_create(
        ...
        let mut request: api::Reply = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.replies().create(
            request,
            opt.value_of("file-id").unwrap_or(""),
            opt.value_of("comment-id").unwrap_or(""),
        );
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["include-deleted", "page-size", "page-token"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "drive3-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/drive3", config_dir))
        .build()
        .await
        .unwrap();
