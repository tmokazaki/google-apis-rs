...
use google_playintegrity1::{api, yup_oauth2, Error};
...
{
    async fn _methods_decode_integrity_token(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(key, &vec!["integrity-token"]);
        ...
        let mut request: api::DecodeIntegrityTokenRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .methods()
            .decode_integrity_token(request, opt.value_of("package-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("methods", Some(opt)) => match opt.subcommand() {
                ("decode-integrity-token", Some(opt)) => {
                    call_result = self
                        ._methods_decode_integrity_token(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "playintegrity1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/playintegrity1", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::PlayIntegrity::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("methods", "methods: 'decode-integrity-token'", vec![
            ("decode-integrity-token",
                    Some(r##"Decodes the integrity token and returns the token payload."##),
                    "Details at http://byron.github.io/google-apis-rs/google_playintegrity1_cli/methods_decode-integrity-token",
                  vec![
                    (Some(r##"package-name"##),
                     None,
                     Some(r##" Package name of the app the attached integrity token belongs to."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
