...
use google_policytroubleshooter1::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::PolicyTroubleshooter<C>,
    gp: Vec<&'static str>,
    ...
    C: apis_common::Connector,
{
    async fn _iam_troubleshoot(
            ...
            .iter()
        {
            ...
            {
                "access-tuple.full-resource-name" => Some((
        ...
        let mut request: api::GoogleCloudPolicytroubleshooterV1TroubleshootIamPolicyRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.iam().troubleshoot(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("iam", Some(opt)) => match opt.subcommand() {
                ("troubleshoot", Some(opt)) => {
                    call_result = self._iam_troubleshoot(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("iam".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "policytroubleshooter1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/policytroubleshooter1", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::PolicyTroubleshooter::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("iam", "methods: 'troubleshoot'", vec![
            ("troubleshoot",
                    Some(r##"Checks whether a principal has a specific permission for a specific resource, and explains why the principal does or does not have that permission."##),
                    "Details at http://byron.github.io/google-apis-rs/google_policytroubleshooter1_cli/iam_troubleshoot",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
    ...
    let mut app = App::new("policytroubleshooter1")
           ...
           .about("")
           .after_help("All documentation details can be found at http://byron.github.io/google-apis-rs/google_policytroubleshooter1_cli")
