...
use google_blogger3::{api, yup_oauth2, Error};
...
{
    async fn _blog_user_infos_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.blog_user_infos().get(
            opt.value_of("user-id").unwrap_or(""),
            opt.value_of("blog-id").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            ...
            .iter()
        {
            ...
            match key {
                "max-posts" => {
                    call = call.max_posts(
                        value
                            .map(|v| arg_from_str(v, err, "max-posts", "uint32"))
                            .unwrap_or(0),
                    );
                }
                _ => {
    ...
    async fn _blogs_get_by_url(
        ...
        let mut call = self
            .hub
            .blogs()
            .get_by_url(opt.value_of("url").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _blogs_list_by_user(
        ...
        let mut call = self
            .hub
            .blogs()
            .list_by_user(opt.value_of("user-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                ...
                "fetch-user-info" => {
                    call = call.fetch_user_info(
                        value
                            .map(|v| arg_from_str(v, err, "fetch-user-info", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["fetch-user-info", "role", "status", "view"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _comments_approve(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.comments().approve(
            opt.value_of("blog-id").unwrap_or(""),
    ...
    async fn _comments_list(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "fetch-bodies" => {
                    call = call.fetch_bodies(
                        value
                            .map(|v| arg_from_str(v, err, "fetch-bodies", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "end-date" => {
    ...
    async fn _comments_list_by_blog(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .comments()
            .list_by_blog(opt.value_of("blog-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _comments_remove_content(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.comments().remove_content(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
            opt.value_of("comment-id").unwrap_or(""),
        );
    ...
    async fn _page_views_get(
        ...
        let mut call = self
            .hub
            .page_views()
            .get(opt.value_of("blog-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _pages_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.pages().delete(
            opt.value_of("blog-id").unwrap_or(""),
            ...
            .iter()
        {
            ...
            match key {
                "use-trash" => {
                    call = call.use_trash(
                        value
                            .map(|v| arg_from_str(v, err, "use-trash", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                let mut v = Vec::new();
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(["use-trash"].iter().map(|v| *v));
                                v
                            }));
                    }
                }
    ...
    async fn _pages_get(
        ...
        let mut call = self.hub.pages().get(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("page-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _pages_insert(
        ...
        let mut request: api::Page = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .pages()
            .insert(request, opt.value_of("blog-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                "is-draft" => {
                    call = call.is_draft(
                        value
                            .map(|v| arg_from_str(v, err, "is-draft", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
    ...
    async fn _pages_patch(
        ...
        let mut request: api::Page = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.pages().patch(
            request,
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("page-id").unwrap_or(""),
        );
            ...
            .iter()
        {
            ...
            match key {
                "revert" => {
                    call = call.revert(
                        value
                            .map(|v| arg_from_str(v, err, "revert", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "publish" => {
                    call = call.publish(
                        value
                            .map(|v| arg_from_str(v, err, "publish", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                let mut v = Vec::new();
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(["publish", "revert"].iter().map(|v| *v));
                                v
                            }));
                    }
                }
    ...
    async fn _pages_publish(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.pages().publish(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("page-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _pages_revert(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.pages().revert(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("page-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _pages_update(
        ...
        let mut request: api::Page = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.pages().update(
            request,
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("page-id").unwrap_or(""),
        );
    ...
    async fn _post_user_infos_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.post_user_infos().get(
            opt.value_of("user-id").unwrap_or(""),
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
            ...
            .iter()
        {
            ...
            match key {
                "max-comments" => {
                    call = call.max_comments(
                        value
                            .map(|v| arg_from_str(v, err, "max-comments", "uint32"))
                            .unwrap_or(0),
                    );
                }
                _ => {
    ...
    async fn _post_user_infos_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.post_user_infos().list(
            opt.value_of("user-id").unwrap_or(""),
            opt.value_of("blog-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _posts_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.posts().delete(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _posts_get(
        ...
        let mut call = self.hub.posts().get(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
        for parg in opt
            ...
            .iter()
        {
            ...
            match key {
                ...
                "fetch-images" => {
                    call = call.fetch_images(
                        value
                            .map(|v| arg_from_str(v, err, "fetch-images", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "fetch-body" => {
                    call = call.fetch_body(
                        value
                            .map(|v| arg_from_str(v, err, "fetch-body", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["fetch-body", "fetch-images", "max-comments", "view"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _posts_get_by_path(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.posts().get_by_path(
            opt.value_of("blog-id").unwrap_or(""),
    ...
    async fn _posts_insert(
        ...
        let mut request: api::Post = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .posts()
            .insert(request, opt.value_of("blog-id").unwrap_or(""));
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                ...
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(
                                    ["fetch-body", "fetch-images", "is-draft"]
                                        .iter()
                                        .map(|v| *v),
                                );
                                v
    ...
    async fn _posts_list(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "sort-option" => {
                    call = call.sort_option(value.unwrap_or(""));
                }
                "page-token" => {
                    call = call.page_token(value.unwrap_or(""));
                }
    ...
    async fn _posts_patch(
        ...
        let mut request: api::Post = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.posts().patch(
            request,
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
    ...
    async fn _posts_publish(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.posts().publish(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _posts_revert(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.posts().revert(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _posts_search(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.posts().search(
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("q").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                let mut v = Vec::new();
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(["fetch-bodies", "order-by"].iter().map(|v| *v));
                                v
                            }));
                    }
                }
    ...
    async fn _posts_update(
        ...
        let mut request: api::Post = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.posts().update(
            request,
            opt.value_of("blog-id").unwrap_or(""),
            opt.value_of("post-id").unwrap_or(""),
        );
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("blog-user-infos", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._blog_user_infos_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("blog-user-infos".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("blogs", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._blogs_get(opt, dry_run, &mut err).await;
                }
                ("get-by-url", Some(opt)) => {
                    call_result = self._blogs_get_by_url(opt, dry_run, &mut err).await;
                }
                ("list-by-user", Some(opt)) => {
                    call_result = self._blogs_list_by_user(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("blogs".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("comments", Some(opt)) => match opt.subcommand() {
                ("approve", Some(opt)) => {
                    call_result = self._comments_approve(opt, dry_run, &mut err).await;
                }
                ("delete", Some(opt)) => {
                ...
                ("list-by-blog", Some(opt)) => {
                    call_result = self._comments_list_by_blog(opt, dry_run, &mut err).await;
                }
                ("mark-as-spam", Some(opt)) => {
                    call_result = self._comments_mark_as_spam(opt, dry_run, &mut err).await;
                }
                ("remove-content", Some(opt)) => {
                    call_result = self._comments_remove_content(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
            ...
            ("page-views", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._page_views_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("page-views".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("pages", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                    call_result = self._pages_delete(opt, dry_run, &mut err).await;
                }
                ("get", Some(opt)) => {
                    call_result = self._pages_get(opt, dry_run, &mut err).await;
                }
                ("insert", Some(opt)) => {
                    call_result = self._pages_insert(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
                    call_result = self._pages_list(opt, dry_run, &mut err).await;
                }
                ("patch", Some(opt)) => {
                    call_result = self._pages_patch(opt, dry_run, &mut err).await;
                }
                ("publish", Some(opt)) => {
                    call_result = self._pages_publish(opt, dry_run, &mut err).await;
                }
                ("revert", Some(opt)) => {
                    call_result = self._pages_revert(opt, dry_run, &mut err).await;
                }
                ("update", Some(opt)) => {
                    call_result = self._pages_update(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("pages".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("post-user-infos", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._post_user_infos_get(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
                    call_result = self._post_user_infos_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("post-user-infos".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("posts", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                    call_result = self._posts_delete(opt, dry_run, &mut err).await;
                }
                ("get", Some(opt)) => {
                    call_result = self._posts_get(opt, dry_run, &mut err).await;
                }
                ("get-by-path", Some(opt)) => {
                    call_result = self._posts_get_by_path(opt, dry_run, &mut err).await;
                }
                ("insert", Some(opt)) => {
                    call_result = self._posts_insert(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
                    call_result = self._posts_list(opt, dry_run, &mut err).await;
                }
                ("patch", Some(opt)) => {
                    call_result = self._posts_patch(opt, dry_run, &mut err).await;
                }
                ("publish", Some(opt)) => {
                    call_result = self._posts_publish(opt, dry_run, &mut err).await;
                }
                ("revert", Some(opt)) => {
                    call_result = self._posts_revert(opt, dry_run, &mut err).await;
                }
                ("search", Some(opt)) => {
                    call_result = self._posts_search(opt, dry_run, &mut err).await;
                }
                ("update", Some(opt)) => {
                    call_result = self._posts_update(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("posts".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "blogger3-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/blogger3", config_dir))
        .build()
        .await
        .unwrap();
