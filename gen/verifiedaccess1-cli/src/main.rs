...
use google_verifiedaccess1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _challenge_create(
        ...
        let mut request: api::Empty = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.challenge().create(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _challenge_verify(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "challenge-response.signature" => Some((
        ...
        let mut request: api::VerifyChallengeResponseRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.challenge().verify(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("challenge", Some(opt)) => match opt.subcommand() {
                ("create", Some(opt)) => {
                    call_result = self._challenge_create(opt, dry_run, &mut err).await;
                }
                ("verify", Some(opt)) => {
                    call_result = self._challenge_verify(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("challenge".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "verifiedaccess1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/verifiedaccess1", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::Verifiedaccess::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("challenge", "methods: 'create' and 'verify'", vec![
            ("create",
                    Some(r##"CreateChallenge API"##),
                    "Details at http://byron.github.io/google-apis-rs/google_verifiedaccess1_cli/challenge_create",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("verify",
                    Some(r##"VerifyChallengeResponse API"##),
                    "Details at http://byron.github.io/google-apis-rs/google_verifiedaccess1_cli/challenge_verify",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
