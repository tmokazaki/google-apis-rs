...
use google_sts1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _methods_token(
        ...
        let mut request: api::GoogleIdentityStsV1ExchangeTokenRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.methods().token(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "sts1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/sts1", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::CloudSecurityToken::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
        ...
        vec![(
            ...
            vec![
                (
                    ...
                    Some(r##"r"##),
                    Some(
                        r##"Set various fields of the request structure, matching the key=value form"##,
                    ),
                    Some(true),
                    Some(true),
                ),
