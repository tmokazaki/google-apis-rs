...
use google_adexperiencereport1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _violating_sites_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("violating-sites", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._violating_sites_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("violating-sites".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "adexperiencereport1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/adexperiencereport1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("sites", "methods: 'get'", vec![
            ("get",
                    Some(r##"Gets a site's Ad Experience Report summary."##),
                    "Details at http://byron.github.io/google-apis-rs/google_adexperiencereport1_cli/sites_get",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("list",
                    Some(r##"Lists sites that are failing in the Ad Experience Report on at least one platform."##),
                    "Details at http://byron.github.io/google-apis-rs/google_adexperiencereport1_cli/violating-sites_list",
                  vec![
                    (Some(r##"v"##),
                     Some(r##"p"##),
                     Some(r##"Set various optional parameters, matching the key=value form"##),
