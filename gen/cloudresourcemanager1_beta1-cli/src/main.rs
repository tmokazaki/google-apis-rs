...
use google_cloudresourcemanager1_beta1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _organizations_list(
        ...
        let mut call = self.hub.organizations().list();
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _organizations_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "owner.directory-customer-id" => Some((
        ...
        let mut request: api::Organization = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .organizations()
    ...
    async fn _projects_create(
            ...
            .iter()
        {
            ...
            match key {
                "use-legacy-stack" => {
                    call = call.use_legacy_stack(
                        value
                            .map(|v| arg_from_str(v, err, "use-legacy-stack", "boolean"))
                            .unwrap_or(false),
                    );
                }
                _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudresourcemanager1-beta1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/cloudresourcemanager1-beta1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("organizations", "methods: 'get', 'get-iam-policy', 'list', 'set-iam-policy', 'test-iam-permissions' and 'update'", vec![
            ...
            ("get-iam-policy",
                    Some(r##"Gets the access control policy for an Organization resource. May be empty if no such policy or resource exists. The `resource` field should be the organization's resource name, e.g. "organizations/123"."##),
            ...
            ("set-iam-policy",
                    Some(r##"Sets the access control policy on an Organization resource. Replaces any existing policy. The `resource` field should be the organization's resource name, e.g. "organizations/123"."##),
            ...
            ("test-iam-permissions",
                    Some(r##"Returns permissions that a caller has on the specified Organization. The `resource` field should be the organization's resource name, e.g. "organizations/123"."##),
            ...
            ("get-iam-policy",
                    Some(r##"Returns the IAM access control policy for the specified Project. Permission is denied if the policy or the resource does not exist. For additional information about resource structure and identification, see [Resource Names](/apis/design/resource_names)."##),
            ...
            ("test-iam-permissions",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_cloudresourcemanager1_beta1_cli/projects_test-iam-permissions",
                  vec![
                    (Some(r##"resource"##),
                     None,
                     Some(r##"REQUIRED: The resource for which the policy detail is being requested. See [Resource names](https://cloud.google.com/apis/design/resource_names) for the appropriate value for this field."##),
