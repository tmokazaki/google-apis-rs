...
use google_acceleratedmobilepageurl1::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::Acceleratedmobilepageurl<C>,
    gp: Vec<&'static str>,
    ...
    C: apis_common::Connector,
{
    async fn _amp_urls_batch_get(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["lookup-strategy", "urls"]);
        ...
        let mut request: api::BatchGetAmpUrlsRequest =
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("amp-urls", Some(opt)) => match opt.subcommand() {
                ("batch-get", Some(opt)) => {
                    call_result = self._amp_urls_batch_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("amp-urls".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "acceleratedmobilepageurl1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/acceleratedmobilepageurl1", config_dir))
        .build()
        .await
        .unwrap();
