...
use google_texttospeech1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _projects_locations_synthesize_long_audio(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "audio-config.effects-profile-id" => Some((
                ...
                "audio-config.sample-rate-hertz" => Some((
                ...
                "audio-config.speaking-rate" => Some((
                ...
                "audio-config.volume-gain-db" => Some((
                ...
                "voice.custom-voice.model" => Some((
                    "voice.customVoice.model",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "voice.custom-voice.reported-usage" => Some((
                    "voice.customVoice.reportedUsage",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "voice.language-code" => Some((
                    "voice.languageCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
        ...
        let mut request: api::SynthesizeLongAudioRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_synthesize_long_audio(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _text_synthesize(
        ...
        let mut request: api::SynthesizeSpeechRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.text().synthesize(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _voices_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.voices().list();
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("projects", Some(opt)) => match opt.subcommand() {
                ...
                ("locations-synthesize-long-audio", Some(opt)) => {
                    call_result = self
                        ._projects_locations_synthesize_long_audio(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
            ...
            ("text", Some(opt)) => match opt.subcommand() {
                ("synthesize", Some(opt)) => {
                    call_result = self._text_synthesize(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("text".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("voices", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._voices_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("voices".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "texttospeech1-secret.json",
        ...
        let engine = Engine {
            opt,
            hub: api::Texttospeech::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
    ...
    let arg_data = [
        ("operations", "methods: 'cancel' and 'delete'", vec![
            ...
            ("delete",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_texttospeech1_cli/operations_delete",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("projects", "methods: 'locations-operations-get', 'locations-operations-list' and 'locations-synthesize-long-audio'", vec![
            ("locations-operations-get",
                    Some(r##"Gets the latest state of a long-running operation. Clients can use this method to poll the operation result at intervals as recommended by the API service."##),
                    "Details at http://byron.github.io/google-apis-rs/google_texttospeech1_cli/projects_locations-operations-get",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("locations-operations-list",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_texttospeech1_cli/projects_locations-operations-list",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("locations-synthesize-long-audio",
                    Some(r##"Synthesizes long form text asynchronously."##),
                    "Details at http://byron.github.io/google-apis-rs/google_texttospeech1_cli/projects_locations-synthesize-long-audio",
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"The resource states of the request in the form of `projects/*/locations/*`."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("text", "methods: 'synthesize'", vec![
            ("synthesize",
                    Some(r##"Synthesizes speech synchronously: receive results after all text input has been processed."##),
                    "Details at http://byron.github.io/google-apis-rs/google_texttospeech1_cli/text_synthesize",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
            ...
            ("voices", "methods: 'list'", vec![
            ("list",
                    Some(r##"Returns a list of Voice supported for synthesis."##),
                    "Details at http://byron.github.io/google-apis-rs/google_texttospeech1_cli/voices_list",
                  vec![
                    (Some(r##"v"##),
                     Some(r##"p"##),
                     Some(r##"Set various optional parameters, matching the key=value form"##),
