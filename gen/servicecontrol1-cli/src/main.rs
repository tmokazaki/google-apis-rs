...
use google_servicecontrol1::{api, yup_oauth2, Error};
...
{
    async fn _services_allocate_quota(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            .iter()
        {
            ...
            {
                "allocate-operation.consumer-id" => Some((
                    "allocateOperation.consumerId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "allocate-operation.labels" => Some((
                ...
                "allocate-operation.method-name" => Some((
                    "allocateOperation.methodName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "allocate-operation.operation-id" => Some((
                    "allocateOperation.operationId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "allocate-operation.quota-mode" => Some((
                    "allocateOperation.quotaMode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::AllocateQuotaRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .services()
            .allocate_quota(request, opt.value_of("service-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _services_check(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "operation.quota-properties.quota-mode" => Some((
                    "operation.quotaProperties.quotaMode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        match self.opt.subcommand() {
            ("services", Some(opt)) => match opt.subcommand() {
                ("allocate-quota", Some(opt)) => {
                    call_result = self._services_allocate_quota(opt, dry_run, &mut err).await;
                }
                ("check", Some(opt)) => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "servicecontrol1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/servicecontrol1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("services", "methods: 'allocate-quota', 'check' and 'report'", vec![
            ("allocate-quota",
                    Some(r##"Attempts to allocate quota for the specified consumer. It should be called before the operation is executed. This method requires the `servicemanagement.services.quota` permission on the specified service. For more information, see [Cloud IAM](https://cloud.google.com/iam). **NOTE:** The client **must** fail-open on server errors `INTERNAL`, `UNKNOWN`, `DEADLINE_EXCEEDED`, and `UNAVAILABLE`. To ensure system reliability, the server may inject these errors to prohibit any hard dependency on the quota functionality."##),
                    "Details at http://byron.github.io/google-apis-rs/google_servicecontrol1_cli/services_allocate-quota",
                  vec![
                    (Some(r##"service-name"##),
                     None,
                     Some(r##"Name of the service as specified in the service configuration. For example, `"pubsub.googleapis.com"`. See google.api.Service for the definition of a service name."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("check",
                    Some(r##"Checks whether an operation on a service should be allowed to proceed based on the configuration of the service and related policies. It must be called before the operation is executed. If feasible, the client should cache the check results and reuse them for 60 seconds. In case of any server errors, the client should rely on the cached results for much longer time to avoid outage. WARNING: There is general 60s delay for the configuration and policy propagation, therefore callers MUST NOT depend on the `Check` method having the latest policy information. NOTE: the CheckRequest has the size limit (wire-format byte size) of 1MB. This method requires the `servicemanagement.services.check` permission on the specified service. For more information, see [Cloud IAM](https://cloud.google.com/iam)."##),
