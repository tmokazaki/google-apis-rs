...
use google_tasks1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _tasklists_delete(
        ...
        let mut call = self
            .hub
            .tasklists()
            .delete(opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tasklists_get(
        ...
        let mut call = self
            .hub
            .tasklists()
            .get(opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tasklists_insert(
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["etag", "id", "kind", "self-link", "title", "updated"],
                    );
        ...
        let mut request: api::TaskList = serde_json::value::from_value(object).unwrap();
    ...
    async fn _tasklists_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.tasklists().list();
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tasklists_patch(
        ...
        let mut call = self
            .hub
            .tasklists()
            .patch(request, opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tasklists_update(
        ...
        let mut call = self
            .hub
            .tasklists()
            .update(request, opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tasks_clear(
        ...
        let mut call = self
            .hub
            .tasks()
            .clear(opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tasks_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.tasks().delete(
            opt.value_of("tasklist").unwrap_or(""),
            opt.value_of("task").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _tasks_get(
        ...
        let mut call = self.hub.tasks().get(
            opt.value_of("tasklist").unwrap_or(""),
            opt.value_of("task").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _tasks_insert(
        ...
        let mut request: api::Task = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .tasks()
            .insert(request, opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                "previous" => {
                    call = call.previous(value.unwrap_or(""));
                }
                "parent" => {
    ...
    async fn _tasks_list(
        ...
        let mut call = self
            .hub
            .tasks()
            .list(opt.value_of("tasklist").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                ...
                "show-completed" => {
                    call = call.show_completed(
                        value
                            .map(|v| arg_from_str(v, err, "show-completed", "boolean"))
                            .unwrap_or(false),
                    );
                }
                "page-token" => {
                ...
                "due-max" => {
                    call = call.due_max(value.unwrap_or(""));
                }
                "completed-min" => {
                    call = call.completed_min(value.unwrap_or(""));
                }
                "completed-max" => {
                    call = call.completed_max(value.unwrap_or(""));
                }
                _ => {
                    let mut found = false;
                    for param in &self.gp {
    ...
    async fn _tasks_move(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.tasks().move_(
            opt.value_of("tasklist").unwrap_or(""),
            opt.value_of("task").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("tasklists", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                    call_result = self._tasklists_delete(opt, dry_run, &mut err).await;
                }
                ("get", Some(opt)) => {
                    call_result = self._tasklists_get(opt, dry_run, &mut err).await;
                }
                ("insert", Some(opt)) => {
                    call_result = self._tasklists_insert(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
                    call_result = self._tasklists_list(opt, dry_run, &mut err).await;
                }
                ("patch", Some(opt)) => {
                    call_result = self._tasklists_patch(opt, dry_run, &mut err).await;
                }
                ("update", Some(opt)) => {
                    call_result = self._tasklists_update(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("tasklists".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("tasks", Some(opt)) => match opt.subcommand() {
                ("clear", Some(opt)) => {
                    call_result = self._tasks_clear(opt, dry_run, &mut err).await;
                }
                ("delete", Some(opt)) => {
                    call_result = self._tasks_delete(opt, dry_run, &mut err).await;
                }
                ("get", Some(opt)) => {
                    call_result = self._tasks_get(opt, dry_run, &mut err).await;
                }
                ("insert", Some(opt)) => {
                    call_result = self._tasks_insert(opt, dry_run, &mut err).await;
                }
                ("list", Some(opt)) => {
                    call_result = self._tasks_list(opt, dry_run, &mut err).await;
                }
                ("move", Some(opt)) => {
                    call_result = self._tasks_move(opt, dry_run, &mut err).await;
                }
                ("patch", Some(opt)) => {
                    call_result = self._tasks_patch(opt, dry_run, &mut err).await;
                }
                ("update", Some(opt)) => {
                    call_result = self._tasks_update(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("tasks".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "tasks1-secret.json",
...
async fn main() {
    ...
    let arg_data = [
        ("tasklists", "methods: 'delete', 'get', 'insert', 'list', 'patch' and 'update'", vec![
            ...
            ("get",
                    Some(r##"Returns the authenticated user's specified task list."##),
                    "Details at http://byron.github.io/google-apis-rs/google_tasks1_cli/tasklists_get",
                  vec![
                    (Some(r##"tasklist"##),
                     None,
                     Some(r##"Task list identifier."##),
            ...
            ("update",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_tasks1_cli/tasklists_update",
                  vec![
                    (Some(r##"tasklist"##),
                     None,
                     Some(r##"Task list identifier."##),
            ...
            ("get",
                    Some(r##"Returns the specified task."##),
                    "Details at http://byron.github.io/google-apis-rs/google_tasks1_cli/tasks_get",
                  vec![
                    (Some(r##"tasklist"##),
                     None,
                     Some(r##"Task list identifier."##),
            ...
            ("patch",
                    Some(r##"Updates the specified task. This method supports patch semantics."##),
                    "Details at http://byron.github.io/google-apis-rs/google_tasks1_cli/tasks_patch",
                  vec![
                    (Some(r##"tasklist"##),
                     None,
                     Some(r##"Task list identifier."##),
            ...
            ("update",
                    Some(r##"Updates the specified task."##),
                    "Details at http://byron.github.io/google-apis-rs/google_tasks1_cli/tasks_update",
                  vec![
                    (Some(r##"tasklist"##),
                     None,
                     Some(r##"Task list identifier."##),
