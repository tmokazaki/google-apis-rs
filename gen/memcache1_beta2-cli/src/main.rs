...
use google_memcache1_beta2::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::CloudMemorystoreForMemcached<C>,
    gp: Vec<&'static str>,
    ...
    C: apis_common::Connector,
{
    ...
    async fn _projects_locations_instances_apply_parameters(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::ApplyParametersRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_instances_apply_parameters(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_instances_apply_software_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::ApplySoftwareUpdateRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_instances_apply_software_update(
                request,
    ...
    async fn _projects_locations_instances_create(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "node-config.memory-size-mb" => Some((
    ...
    async fn _projects_locations_instances_update_parameters(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["id", "parameters", "params", "update-mask"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::UpdateParametersRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_instances_update_parameters(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_instances_upgrade(
        ...
        let mut request: api::GoogleCloudMemcacheV1beta2UpgradeInstanceRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ...
                ("locations-instances-apply-parameters", Some(opt)) => {
                    call_result = self
                        ._projects_locations_instances_apply_parameters(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-instances-apply-software-update", Some(opt)) => {
                    call_result = self
                        ._projects_locations_instances_apply_software_update(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("locations-instances-update-parameters", Some(opt)) => {
                    call_result = self
                        ._projects_locations_instances_update_parameters(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "memcache1-beta2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/memcache1-beta2", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-instances-apply-parameters', 'locations-instances-apply-software-update', 'locations-instances-create', 'locations-instances-delete', 'locations-instances-get', 'locations-instances-list', 'locations-instances-patch', 'locations-instances-reschedule-maintenance', 'locations-instances-update-parameters', 'locations-instances-upgrade', 'locations-list', 'locations-operations-cancel', 'locations-operations-delete', 'locations-operations-get' and 'locations-operations-list'", vec![
            ("locations-get",
            ...
            ("locations-instances-apply-parameters",
                    Some(r##"`ApplyParameters` restarts the set of specified nodes in order to update them to the current set of parameters for the Memcached Instance."##),
                    "Details at http://byron.github.io/google-apis-rs/google_memcache1_beta2_cli/projects_locations-instances-apply-parameters",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Resource name of the Memcached instance for which parameter group updates should be applied."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("locations-instances-apply-software-update",
                    Some(r##"Updates software on the selected nodes of the Instance."##),
                    "Details at http://byron.github.io/google-apis-rs/google_memcache1_beta2_cli/projects_locations-instances-apply-software-update",
                  vec![
                    (Some(r##"instance"##),
                     None,
                     Some(r##"Required. Resource name of the Memcached instance for which software update should be applied."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("locations-instances-create",
                    Some(r##"Creates a new Instance in a given location."##),
            ...
            ("locations-instances-delete",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Memcached instance resource name in the format: `projects/{project_id}/locations/{location_id}/instances/{instance_id}` where `location_id` refers to a GCP region"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("locations-instances-list",
                    Some(r##"Lists Instances in a given location."##),
            ...
            ("locations-instances-patch",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Unique name of the resource in this scope including project and location using the form: `projects/{project_id}/locations/{location_id}/instances/{instance_id}` Note: Memcached instances are managed and addressed at the regional level so `location_id` here refers to a Google Cloud region; however, users may choose which zones Memcached nodes should be provisioned in within an instance. Refer to zones field for more details."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("locations-instances-reschedule-maintenance",
                    Some(r##"Performs the apply phase of the RescheduleMaintenance verb."##),
                    "Details at http://byron.github.io/google-apis-rs/google_memcache1_beta2_cli/projects_locations-instances-reschedule-maintenance",
                  vec![
                    (Some(r##"instance"##),
                     None,
            ...
            ("locations-instances-update-parameters",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Resource name of the Memcached instance for which the parameters should be updated."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("locations-instances-upgrade",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_memcache1_beta2_cli/projects_locations-instances-upgrade",
                  vec![
                    (Some(r##"name"##),
                     None,
