...
use google_fcmdata1_beta1::{api, yup_oauth2, Error};
...
{
    async fn _projects_android_apps_delivery_data_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .projects()
            .android_apps_delivery_data_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("android-apps-delivery-data-list", Some(opt)) => {
                    call_result = self
                        ._projects_android_apps_delivery_data_list(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "fcmdata1-beta1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/fcmdata1-beta1", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::Fcmdata::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'android-apps-delivery-data-list'", vec![
            ("android-apps-delivery-data-list",
                    Some(r##"List aggregate delivery data for the given Android application."##),
                    "Details at http://byron.github.io/google-apis-rs/google_fcmdata1_beta1_cli/projects_android-apps-delivery-data-list",
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"Required. The application for which to list delivery data. Format: `projects/{project_id}/androidApps/{app_id}`"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
