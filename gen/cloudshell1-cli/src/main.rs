...
use google_cloudshell1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _users_environments_add_public_key(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::AddPublicKeyRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .users()
            .environments_add_public_key(request, opt.value_of("environment").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _users_environments_authorize(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["access-token", "expire-time", "id-token"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::AuthorizeEnvironmentRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .users()
            .environments_authorize(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _users_environments_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .users()
            .environments_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _users_environments_remove_public_key(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::RemovePublicKeyRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .users()
            .environments_remove_public_key(request, opt.value_of("environment").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _users_environments_start(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["access-token", "public-keys"]);
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
                        value.map(|v| v.to_string()),
        ...
        let mut request: api::StartEnvironmentRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .users()
            .environments_start(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ...
            ("users", Some(opt)) => match opt.subcommand() {
                ("environments-add-public-key", Some(opt)) => {
                    call_result = self
                        ._users_environments_add_public_key(opt, dry_run, &mut err)
                        .await;
                }
                ...
                ("environments-remove-public-key", Some(opt)) => {
                    call_result = self
                        ._users_environments_remove_public_key(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudshell1-secret.json",
