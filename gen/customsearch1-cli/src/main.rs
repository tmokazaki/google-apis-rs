...
use google_customsearch1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
            ...
            .iter()
        {
            ...
            match key {
                "start" => {
                    call = call.start(
                        value
                            .map(|v| arg_from_str(v, err, "start", "uint32"))
                            .unwrap_or(0),
                    );
                }
                "sort" => {
                ...
                "snippet-length" => {
                    call = call.snippet_length(
                        value
                            .map(|v| arg_from_str(v, err, "snippet-length", "int32"))
                            .unwrap_or(-0),
                    );
                }
                "site-search-filter" => {
                ...
                "num" => {
                    call = call.num(
                        value
                            .map(|v| arg_from_str(v, err, "num", "int32"))
                            .unwrap_or(-0),
                    );
                }
                "lr" => {
    ...
    async fn _cse_siterestrict_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("cse", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("cse".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "customsearch1-secret.json",
