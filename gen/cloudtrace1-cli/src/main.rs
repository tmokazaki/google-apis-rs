...
use google_cloudtrace1::{api, yup_oauth2, Error};
...
{
    async fn _projects_patch_traces(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::Traces = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .patch_traces(request, opt.value_of("project-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_traces_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.projects().traces_get(
            opt.value_of("project-id").unwrap_or(""),
            opt.value_of("trace-id").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _projects_traces_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .traces_list(opt.value_of("project-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("patch-traces", Some(opt)) => {
                    call_result = self._projects_patch_traces(opt, dry_run, &mut err).await;
                }
                ("traces-get", Some(opt)) => {
                    call_result = self._projects_traces_get(opt, dry_run, &mut err).await;
                }
                ("traces-list", Some(opt)) => {
                    call_result = self._projects_traces_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudtrace1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/cloudtrace1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'patch-traces', 'traces-get' and 'traces-list'", vec![
            ...
            ("traces-list",
                    Some(r##"Returns a list of traces that match the specified filter conditions."##),
