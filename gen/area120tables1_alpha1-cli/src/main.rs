...
use google_area120tables1_alpha1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _tables_rows_batch_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::BatchCreateRowsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .tables()
            .rows_batch_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_batch_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::BatchDeleteRowsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .tables()
            .rows_batch_delete(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_batch_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::BatchUpdateRowsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .tables()
            .rows_batch_update(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_create(
        ...
        let mut request: api::Row = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .tables()
            .rows_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_delete(
        ...
        let mut call = self
            .hub
            .tables()
            .rows_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_get(
        ...
        let mut call = self
            .hub
            .tables()
            .rows_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_list(
        ...
        let mut call = self
            .hub
            .tables()
            .rows_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _tables_rows_patch(
        ...
        let mut call = self
            .hub
            .tables()
            .rows_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("workspaces", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("workspaces".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "area120tables1-alpha1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/area120tables1-alpha1", config_dir))
        .build()
        .await
        .unwrap();
