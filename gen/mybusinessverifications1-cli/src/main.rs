...
use google_mybusinessverifications1::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::MyBusinessVerifications<C>,
    gp: Vec<&'static str>,
...
{
    async fn _locations_fetch_verification_options(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            .iter()
        {
            ...
            {
                "context.address.address-lines" => Some((
                ...
                "context.address.administrative-area" => Some((
                    "context.address.administrativeArea",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.address.language-code" => Some((
                    "context.address.languageCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.address.locality" => Some((
                ...
                "context.address.organization" => Some((
                    "context.address.organization",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.address.postal-code" => Some((
                ...
                "context.address.recipients" => Some((
                ...
                "context.address.region-code" => Some((
                    "context.address.regionCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.address.revision" => Some((
                    "context.address.revision",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.address.sorting-code" => Some((
                    "context.address.sortingCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.address.sublocality" => Some((
                    "context.address.sublocality",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::FetchVerificationOptionsRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .locations()
            .fetch_verification_options(request, opt.value_of("location").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _locations_get_voice_of_merchant_state(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .locations()
            .get_voice_of_merchant_state(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _locations_verifications_complete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::CompleteVerificationRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .locations()
            .verifications_complete(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _locations_verifications_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .locations()
            .verifications_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _locations_verify(
        ...
        let mut request: api::VerifyLocationRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .locations()
        ...
        match self.opt.subcommand() {
            ("locations", Some(opt)) => match opt.subcommand() {
                ("fetch-verification-options", Some(opt)) => {
                    call_result = self
                        ._locations_fetch_verification_options(opt, dry_run, &mut err)
                        .await;
                }
                ("get-voice-of-merchant-state", Some(opt)) => {
                    call_result = self
                        ._locations_get_voice_of_merchant_state(opt, dry_run, &mut err)
                        .await;
                }
                ("verifications-complete", Some(opt)) => {
                    call_result = self
                        ._locations_verifications_complete(opt, dry_run, &mut err)
                        .await;
                }
                ("verifications-list", Some(opt)) => {
                    call_result = self
                        ._locations_verifications_list(opt, dry_run, &mut err)
                        .await;
                }
                ("verify", Some(opt)) => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "mybusinessverifications1-secret.json",
