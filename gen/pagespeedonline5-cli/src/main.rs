...
use google_pagespeedonline5::{api, yup_oauth2, Error};
...
{
    async fn _pagespeedapi_runpagespeed(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .pagespeedapi()
            .runpagespeed(opt.value_of("url").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                "utm-source" => {
                    call = call.utm_source(value.unwrap_or(""));
                }
                "utm-campaign" => {
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("pagespeedapi", Some(opt)) => match opt.subcommand() {
                ("runpagespeed", Some(opt)) => {
                    call_result = self
                        ._pagespeedapi_runpagespeed(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("pagespeedapi".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "pagespeedonline5-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/pagespeedonline5", config_dir))
        .build()
        .await
        .unwrap();
        ...
        ("pagespeedapi", "methods: 'runpagespeed'", vec![
            ("runpagespeed",
                  ...
                  vec![
                    (Some(r##"url"##),
                     None,
                     Some(r##"Required. The URL to fetch and analyze"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
