...
use google_cloudtrace2::{api, yup_oauth2, Error};
...
{
    async fn _projects_traces_batch_write(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::BatchWriteSpansRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .traces_batch_write(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_traces_spans_create_span(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "attributes.dropped-attributes-count" => Some((
                ...
                "display-name.truncated-byte-count" => Some((
                    "displayName.truncatedByteCount",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "display-name.value" => Some((
                    "displayName.value",
                    JsonTypeInfo {
                ...
                "links.dropped-links-count" => Some((
                ...
                "same-process-as-parent-span" => Some((
                ...
                "stack-trace.stack-frames.dropped-frames-count" => Some((
                    "stackTrace.stackFrames.droppedFramesCount",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "stack-trace.stack-trace-hash-id" => Some((
                ...
                "time-events.dropped-annotations-count" => Some((
                ...
                "time-events.dropped-message-events-count" => Some((
        ...
        let mut request: api::Span = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .traces_spans_create_span(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("traces-batch-write", Some(opt)) => {
                    call_result = self
                        ._projects_traces_batch_write(opt, dry_run, &mut err)
                        .await;
                }
                ("traces-spans-create-span", Some(opt)) => {
                    call_result = self
                        ._projects_traces_spans_create_span(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudtrace2-secret.json",
        ...
        ("projects", "methods: 'traces-batch-write' and 'traces-spans-create-span'", vec![
            ("traces-batch-write",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. The name of the project where the spans belong. The format is `projects/[PROJECT_ID]`."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
