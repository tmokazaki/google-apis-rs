...
use google_cloudprofiler2::{api, yup_oauth2, Error};
...
{
    async fn _projects_profiles_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::CreateProfileRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .profiles_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_profiles_create_offline(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::Profile = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .profiles_create_offline(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_profiles_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .profiles_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_profiles_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .profiles_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("profiles-create", Some(opt)) => {
                    call_result = self._projects_profiles_create(opt, dry_run, &mut err).await;
                }
                ("profiles-create-offline", Some(opt)) => {
                    call_result = self
                        ._projects_profiles_create_offline(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "cloudprofiler2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/cloudprofiler2", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::CloudProfiler::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
        ...
        ("projects", "methods: 'profiles-create', 'profiles-create-offline', 'profiles-list' and 'profiles-patch'", vec![
            ("profiles-create",
                  ...
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"Parent project to create the profile in."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("profiles-patch",
                  ...
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Output only. Opaque, server-assigned, unique ID for this profile."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
