...
use google_abusiveexperiencereport1::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::AbusiveExperienceReport<C>,
    gp: Vec<&'static str>,
    ...
    C: apis_common::Connector,
{
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "abusiveexperiencereport1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/abusiveexperiencereport1", config_dir))
        .build()
        .await
        .unwrap();
