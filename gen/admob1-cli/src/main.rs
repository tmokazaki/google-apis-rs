...
use google_admob1::{api, yup_oauth2, Error};
...
{
    async fn _accounts_ad_units_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .accounts()
            .ad_units_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_apps_list(
        ...
        let mut call = self
            .hub
            .accounts()
            .apps_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_mediation_report_generate(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            {
                "report-spec.date-range.end-date.day" => Some((
                    "reportSpec.dateRange.endDate.day",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.date-range.end-date.month" => Some((
                    "reportSpec.dateRange.endDate.month",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.date-range.end-date.year" => Some((
                    "reportSpec.dateRange.endDate.year",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.date-range.start-date.day" => Some((
                    "reportSpec.dateRange.startDate.day",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.date-range.start-date.month" => Some((
                    "reportSpec.dateRange.startDate.month",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.date-range.start-date.year" => Some((
                    "reportSpec.dateRange.startDate.year",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.dimensions" => Some((
                    "reportSpec.dimensions",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "report-spec.localization-settings.currency-code" => Some((
                    "reportSpec.localizationSettings.currencyCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.localization-settings.language-code" => Some((
                    "reportSpec.localizationSettings.languageCode",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.max-report-rows" => Some((
                    "reportSpec.maxReportRows",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "report-spec.metrics" => Some((
                    "reportSpec.metrics",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
        ...
        let mut request: api::GenerateMediationReportRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .mediation_report_generate(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _accounts_network_report_generate(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::GenerateNetworkReportRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .accounts()
            .network_report_generate(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("accounts", Some(opt)) => match opt.subcommand() {
                ("ad-units-list", Some(opt)) => {
                    call_result = self._accounts_ad_units_list(opt, dry_run, &mut err).await;
                }
                ("apps-list", Some(opt)) => {
                    call_result = self._accounts_apps_list(opt, dry_run, &mut err).await;
                }
                ("get", Some(opt)) => {
                ...
                ("mediation-report-generate", Some(opt)) => {
                    call_result = self
                        ._accounts_mediation_report_generate(opt, dry_run, &mut err)
                        .await;
                }
                ("network-report-generate", Some(opt)) => {
                    call_result = self
                        ._accounts_network_report_generate(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "admob1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/admob1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("accounts", "methods: 'ad-units-list', 'apps-list', 'get', 'list', 'mediation-report-generate' and 'network-report-generate'", vec![
            ("ad-units-list",
                    Some(r##"List the ad units under the specified AdMob account."##),
                    "Details at http://byron.github.io/google-apis-rs/google_admob1_cli/accounts_ad-units-list",
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"Required. Resource name of the account to list ad units for. Example: accounts/pub-9876543210987654"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("apps-list",
                    Some(r##"List the apps under the specified AdMob account."##),
                    "Details at http://byron.github.io/google-apis-rs/google_admob1_cli/accounts_apps-list",
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"Required. Resource name of the account to list apps for. Example: accounts/pub-9876543210987654"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("get",
                    Some(r##"Gets information about the specified AdMob publisher account."##),
                    "Details at http://byron.github.io/google-apis-rs/google_admob1_cli/accounts_get",
                  vec![
                    (Some(r##"name"##),
                     None,
