...
use google_driveactivity2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _activity_query(
        ...
        let mut request: api::QueryDriveActivityRequest =
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("activity", Some(opt)) => match opt.subcommand() {
                ("query", Some(opt)) => {
                    call_result = self._activity_query(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("activity".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "driveactivity2-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/driveactivity2", config_dir))
        .build()
        .await
        .unwrap();
