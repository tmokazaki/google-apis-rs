...
use google_digitalassetlinks1::{api, yup_oauth2, Error};
...
{
    async fn _assetlinks_bulk_check(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "default-source.android-app.certificate.sha256-fingerprint" => Some((
                    "defaultSource.androidApp.certificate.sha256Fingerprint",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "default-source.android-app.package-name" => Some((
                    "defaultSource.androidApp.packageName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "default-source.web.site" => Some((
                    "defaultSource.web.site",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "default-target.android-app.certificate.sha256-fingerprint" => Some((
                    "defaultTarget.androidApp.certificate.sha256Fingerprint",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "default-target.android-app.package-name" => Some((
                    "defaultTarget.androidApp.packageName",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "default-target.web.site" => Some((
                    "defaultTarget.web.site",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::BulkCheckRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.assetlinks().bulk_check(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            ...
            .iter()
        {
            ...
            match key {
                ...
                "target-android-app-certificate-sha256-fingerprint" => {
                    call =
                        call.target_android_app_certificate_sha256_fingerprint(value.unwrap_or(""));
                }
                "source-web-site" => {
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("assetlinks", Some(opt)) => match opt.subcommand() {
                ("bulk-check", Some(opt)) => {
                    call_result = self._assetlinks_bulk_check(opt, dry_run, &mut err).await;
                }
                ("check", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("assetlinks".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("statements", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._statements_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("statements".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "digitalassetlinks1-secret.json",
...
async fn main() {
    ...
    let arg_data = [
        ("assetlinks", "methods: 'bulk-check' and 'check'", vec![
            ("bulk-check",
                    Some(r##"Send a bundle of statement checks in a single RPC to minimize latency and service load. Statements need not be all for the same source and/or target. We recommend using this method when you need to check more than one statement in a short period of time."##),
                    "Details at http://byron.github.io/google-apis-rs/google_digitalassetlinks1_cli/assetlinks_bulk-check",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
