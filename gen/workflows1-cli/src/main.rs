...
use google_workflows1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _projects_locations_workflows_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::Workflow = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .locations_workflows_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                "workflow-id" => {
                    call = call.workflow_id(value.unwrap_or(""));
                }
                _ => {
                    let mut found = false;
                    for param in &self.gp {
                    ...
                    if !found {
                        err.issues
                            .push(CLIError::UnknownParameter(key.to_string(), {
                                let mut v = Vec::new();
                                v.extend(self.gp.iter().map(|v| *v));
                                v.extend(["workflow-id"].iter().map(|v| *v));
                                v
                            }));
                    }
                }
    ...
    async fn _projects_locations_workflows_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_workflows_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_workflows_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_workflows_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_workflows_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_workflows_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_workflows_list_revisions(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_workflows_list_revisions(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_locations_workflows_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .locations_workflows_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ...
                ("locations-workflows-create", Some(opt)) => {
                    call_result = self
                        ._projects_locations_workflows_create(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-workflows-delete", Some(opt)) => {
                    call_result = self
                        ._projects_locations_workflows_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-workflows-get", Some(opt)) => {
                    call_result = self
                        ._projects_locations_workflows_get(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-workflows-list", Some(opt)) => {
                    call_result = self
                        ._projects_locations_workflows_list(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-workflows-list-revisions", Some(opt)) => {
                    call_result = self
                        ._projects_locations_workflows_list_revisions(opt, dry_run, &mut err)
                        .await;
                }
                ("locations-workflows-patch", Some(opt)) => {
                    call_result = self
                        ._projects_locations_workflows_patch(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "workflows1-secret.json",
        ...
        let engine = Engine {
            opt,
            hub: api::Workflows::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
    ...
    let arg_data = [
        ("projects", "methods: 'locations-get', 'locations-list', 'locations-operations-delete', 'locations-operations-get', 'locations-operations-list', 'locations-workflows-create', 'locations-workflows-delete', 'locations-workflows-get', 'locations-workflows-list', 'locations-workflows-list-revisions' and 'locations-workflows-patch'", vec![
            ...
            ("locations-list",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-list",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("locations-operations-delete",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-operations-delete",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("locations-operations-get",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-operations-get",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("locations-workflows-create",
                    Some(r##"Creates a new workflow. If a workflow with the specified name already exists in the specified project and location, the long running operation returns a ALREADY_EXISTS error."##),
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-workflows-create",
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"Required. Project and location in which the workflow should be created. Format: projects/{project}/locations/{location}"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("locations-workflows-delete",
                    Some(r##"Deletes a workflow with the specified name. This method also cancels and deletes all running executions of the workflow."##),
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-workflows-delete",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Name of the workflow to be deleted. Format: projects/{project}/locations/{location}/workflows/{workflow}"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("locations-workflows-get",
                    Some(r##"Gets details of a single workflow."##),
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-workflows-get",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Name of the workflow for which information should be retrieved. Format: projects/{project}/locations/{location}/workflows/{workflow}"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("locations-workflows-list",
                    Some(r##"Lists workflows in a given project and location. The default order is not specified."##),
                    "Details at http://byron.github.io/google-apis-rs/google_workflows1_cli/projects_locations-workflows-list",
                  vec![
                    (Some(r##"parent"##),
                     None,
                     Some(r##"Required. Project and location from which the workflows should be listed. Format: projects/{project}/locations/{location}"##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
