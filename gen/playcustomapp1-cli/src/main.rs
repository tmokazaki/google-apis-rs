...
use google_playcustomapp1::{api, yup_oauth2, Error};
...
{
    async fn _accounts_custom_apps_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["language-code", "package-name", "title"],
                    );
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("accounts", Some(opt)) => match opt.subcommand() {
                ("custom-apps-create", Some(opt)) => {
                    call_result = self
                        ._accounts_custom_apps_create(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "playcustomapp1-secret.json",
