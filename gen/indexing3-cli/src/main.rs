...
use google_indexing3::{api, yup_oauth2, Error};
...
{
    async fn _url_notifications_get_metadata(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.url_notifications().get_metadata();
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _url_notifications_publish(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion =
                        FieldCursor::did_you_mean(key, &vec!["notify-time", "type", "url"]);
        ...
        let mut request: api::UrlNotification = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.url_notifications().publish(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("url-notifications", Some(opt)) => match opt.subcommand() {
                ("get-metadata", Some(opt)) => {
                    call_result = self
                        ._url_notifications_get_metadata(opt, dry_run, &mut err)
                        .await;
                }
                ("publish", Some(opt)) => {
                    call_result = self
                        ._url_notifications_publish(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "indexing3-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/indexing3", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::Indexing::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("url-notifications", "methods: 'get-metadata' and 'publish'", vec![
            ("get-metadata",
                    Some(r##"Gets metadata about a Web Document. This method can _only_ be used to query URLs that were previously seen in successful Indexing API notifications. Includes the latest `UrlNotification` received via this API."##),
                    "Details at http://byron.github.io/google-apis-rs/google_indexing3_cli/url-notifications_get-metadata",
                  vec![
                    (Some(r##"v"##),
                     Some(r##"p"##),
            ...
            ("publish",
                    Some(r##"Notifies that a URL has been updated or deleted."##),
                    "Details at http://byron.github.io/google-apis-rs/google_indexing3_cli/url-notifications_publish",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
