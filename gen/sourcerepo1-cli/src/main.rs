...
use google_sourcerepo1::{api, yup_oauth2, Error};
...
struct Engine<'n, C> {
    opt: ArgMatches<'n>,
    hub: api::CloudSourceRepositories<C>,
    gp: Vec<&'static str>,
    ...
    C: apis_common::Connector,
{
    ...
    async fn _projects_repos_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            {
                "mirror-config.deploy-key-id" => Some((
                    "mirrorConfig.deployKeyId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "mirror-config.url" => Some((
                    "mirrorConfig.url",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                ...
                "mirror-config.webhook-id" => Some((
                    "mirrorConfig.webhookId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::Repo = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .repos_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .repos_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_get(
        ...
        let mut call = self
            .hub
            .projects()
            .repos_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_get_iam_policy(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .repos_get_iam_policy(opt.value_of("resource").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_list(
        ...
        let mut call = self
            .hub
            .projects()
            .repos_list(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "repo.mirror-config.deploy-key-id" => Some((
                    "repo.mirrorConfig.deployKeyId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "repo.mirror-config.url" => Some((
                    "repo.mirrorConfig.url",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "repo.mirror-config.webhook-id" => Some((
                    "repo.mirrorConfig.webhookId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::UpdateRepoRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .repos_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_set_iam_policy(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .repos_set_iam_policy(request, opt.value_of("resource").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_sync(
        ...
        let mut request: api::SyncRepoRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .repos_sync(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_repos_test_iam_permissions(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .repos_test_iam_permissions(request, opt.value_of("resource").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_update_config(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "project-config.enable-private-key-check" => Some((
                    "projectConfig.enablePrivateKeyCheck",
                    JsonTypeInfo {
                        jtype: JsonType::Boolean,
                        ctype: ComplexType::Pod,
                    },
                )),
                "project-config.name" => Some((
                    "projectConfig.name",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
        ...
        let mut request: api::UpdateProjectConfigRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ...
                ("repos-get-iam-policy", Some(opt)) => {
                    call_result = self
                        ._projects_repos_get_iam_policy(opt, dry_run, &mut err)
                        .await;
                }
                ("repos-list", Some(opt)) => {
                ...
                ("repos-set-iam-policy", Some(opt)) => {
                    call_result = self
                        ._projects_repos_set_iam_policy(opt, dry_run, &mut err)
                        .await;
                }
                ("repos-sync", Some(opt)) => {
                ...
                ("repos-test-iam-permissions", Some(opt)) => {
                    call_result = self
                        ._projects_repos_test_iam_permissions(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "sourcerepo1-secret.json",
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'get-config', 'repos-create', 'repos-delete', 'repos-get', 'repos-get-iam-policy', 'repos-list', 'repos-patch', 'repos-set-iam-policy', 'repos-sync', 'repos-test-iam-permissions' and 'update-config'", vec![
            ...
            ("repos-delete",
                    Some(r##"Deletes a repo."##),
                    "Details at http://byron.github.io/google-apis-rs/google_sourcerepo1_cli/projects_repos-delete",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("repos-get",
                    Some(r##"Returns information about a repo."##),
                    "Details at http://byron.github.io/google-apis-rs/google_sourcerepo1_cli/projects_repos-get",
                  vec![
                    (Some(r##"name"##),
                     None,
