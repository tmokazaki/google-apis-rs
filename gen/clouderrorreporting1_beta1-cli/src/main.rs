...
use google_clouderrorreporting1_beta1::{api, yup_oauth2, Error};
...
{
    async fn _projects_delete_events(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .projects()
            .delete_events(opt.value_of("project-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_events_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .events_list(opt.value_of("project-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_events_report(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            {
                "context.http-request.method" => Some((
                ...
                "context.http-request.referrer" => Some((
                    "context.httpRequest.referrer",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.http-request.remote-ip" => Some((
                    "context.httpRequest.remoteIp",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.http-request.response-status-code" => Some((
                    "context.httpRequest.responseStatusCode",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.http-request.url" => Some((
                ...
                "context.http-request.user-agent" => Some((
                    "context.httpRequest.userAgent",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.report-location.file-path" => Some((
                    "context.reportLocation.filePath",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "context.report-location.function-name" => Some((
                ...
                "context.report-location.line-number" => Some((
                    "context.reportLocation.lineNumber",
                    JsonTypeInfo {
                        jtype: JsonType::Int,
                        ctype: ComplexType::Pod,
                    },
                ...
                "service-context.resource-type" => Some((
        ...
        let mut request: api::ReportedErrorEvent = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .events_report(request, opt.value_of("project-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_group_stats_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .group_stats_list(opt.value_of("project-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
            .iter()
        {
            ...
            match key {
                "timed-count-duration" => {
                    call = call.timed_count_duration(
                        value
                            .map(|v| {
                                arg_from_str(v, err, "timed-count-duration", "google-duration")
                            })
                ...
                "alignment-time" => {
                    call = call.alignment_time(
                        value
                            .map(|v| arg_from_str(v, err, "alignment-time", "google-datetime"))
    ...
    async fn _projects_groups_get(
        ...
        let mut call = self
            .hub
            .projects()
            .groups_get(opt.value_of("group-name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_groups_update(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["group-id", "name", "resolution-status"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::ErrorGroup = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ...
                ("group-stats-list", Some(opt)) => {
                    call_result = self
                        ._projects_group_stats_list(opt, dry_run, &mut err)
                        .await;
                }
                ("groups-get", Some(opt)) => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "clouderrorreporting1-beta1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/clouderrorreporting1-beta1", config_dir))
        .build()
        .await
        .unwrap();
