...
use google_testing1::{api, yup_oauth2, Error};
...
{
    async fn _application_detail_service_get_apk_details(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::FileReference = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .application_detail_service()
            .get_apk_details(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_device_sessions_cancel(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::CancelDeviceSessionRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .device_sessions_cancel(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_device_sessions_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "android-device.android-model-id" => Some((
                    "androidDevice.androidModelId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "android-device.android-version-id" => Some((
                    "androidDevice.androidVersionId",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "android-device.locale" => Some((
                    "androidDevice.locale",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
                )),
                "android-device.orientation" => Some((
                    "androidDevice.orientation",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Pod,
                    },
        ...
        let mut request: api::DeviceSession = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .device_sessions_create(request, opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_device_sessions_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .device_sessions_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_device_sessions_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .device_sessions_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_device_sessions_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .device_sessions_patch(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_test_matrices_cancel(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.projects().test_matrices_cancel(
            opt.value_of("project-id").unwrap_or(""),
            opt.value_of("test-matrix-id").unwrap_or(""),
        );
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
    ...
    async fn _projects_test_matrices_create(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::TestMatrix = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .test_matrices_create(request, opt.value_of("project-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_test_matrices_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.projects().test_matrices_get(
            opt.value_of("project-id").unwrap_or(""),
            opt.value_of("test-matrix-id").unwrap_or(""),
        );
        for parg in opt
    ...
    async fn _test_environment_catalog_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            ...
            .get(opt.value_of("environment-type").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("application-detail-service", Some(opt)) => match opt.subcommand() {
                ("get-apk-details", Some(opt)) => {
                    call_result = self
                        ._application_detail_service_get_apk_details(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "application-detail-service".to_string(),
                    ));
            ...
            ("projects", Some(opt)) => match opt.subcommand() {
                ("device-sessions-cancel", Some(opt)) => {
                    call_result = self
                        ._projects_device_sessions_cancel(opt, dry_run, &mut err)
                        .await;
                }
                ("device-sessions-create", Some(opt)) => {
                    call_result = self
                        ._projects_device_sessions_create(opt, dry_run, &mut err)
                        .await;
                }
                ("device-sessions-get", Some(opt)) => {
                    call_result = self
                        ._projects_device_sessions_get(opt, dry_run, &mut err)
                        .await;
                }
                ("device-sessions-list", Some(opt)) => {
                    call_result = self
                        ._projects_device_sessions_list(opt, dry_run, &mut err)
                        .await;
                }
                ("device-sessions-patch", Some(opt)) => {
                    call_result = self
                        ._projects_device_sessions_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("test-matrices-cancel", Some(opt)) => {
                    call_result = self
                        ._projects_test_matrices_cancel(opt, dry_run, &mut err)
                        .await;
                }
                ("test-matrices-create", Some(opt)) => {
                    call_result = self
                        ._projects_test_matrices_create(opt, dry_run, &mut err)
                        .await;
                }
                ("test-matrices-get", Some(opt)) => {
                    call_result = self
                        ._projects_test_matrices_get(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
            ...
            ("test-environment-catalog", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self
                        ._test_environment_catalog_get(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "test-environment-catalog".to_string(),
                    ));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "testing1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/testing1", config_dir))
        .build()
        .await
        .unwrap();
