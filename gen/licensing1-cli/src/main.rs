...
use google_licensing1::{api, yup_oauth2, Error};
...
{
    async fn _license_assignments_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.license_assignments().delete(
            opt.value_of("product-id").unwrap_or(""),
            opt.value_of("sku-id").unwrap_or(""),
    ...
    async fn _license_assignments_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.license_assignments().get(
            opt.value_of("product-id").unwrap_or(""),
            opt.value_of("sku-id").unwrap_or(""),
            opt.value_of("user-id").unwrap_or(""),
        );
    ...
    async fn _license_assignments_insert(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::LicenseAssignmentInsert =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.license_assignments().insert(
            request,
            opt.value_of("product-id").unwrap_or(""),
            opt.value_of("sku-id").unwrap_or(""),
        );
    ...
    async fn _license_assignments_list_for_product(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.license_assignments().list_for_product(
            opt.value_of("product-id").unwrap_or(""),
    ...
    async fn _license_assignments_list_for_product_and_sku(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let mut call = self.hub.license_assignments().list_for_product_and_sku(
            opt.value_of("product-id").unwrap_or(""),
            opt.value_of("sku-id").unwrap_or(""),
            opt.value_of("customer-id").unwrap_or(""),
        );
    ...
    async fn _license_assignments_patch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::LicenseAssignment = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.license_assignments().patch(
            request,
            opt.value_of("product-id").unwrap_or(""),
            opt.value_of("sku-id").unwrap_or(""),
            opt.value_of("user-id").unwrap_or(""),
    ...
    async fn _license_assignments_update(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::LicenseAssignment = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.license_assignments().update(
            request,
            opt.value_of("product-id").unwrap_or(""),
            opt.value_of("sku-id").unwrap_or(""),
            opt.value_of("user-id").unwrap_or(""),
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("license-assignments", Some(opt)) => match opt.subcommand() {
                ("delete", Some(opt)) => {
                    call_result = self
                        ._license_assignments_delete(opt, dry_run, &mut err)
                        .await;
                }
                ("get", Some(opt)) => {
                    call_result = self._license_assignments_get(opt, dry_run, &mut err).await;
                }
                ("insert", Some(opt)) => {
                    call_result = self
                        ._license_assignments_insert(opt, dry_run, &mut err)
                        .await;
                }
                ("list-for-product", Some(opt)) => {
                    call_result = self
                        ._license_assignments_list_for_product(opt, dry_run, &mut err)
                        .await;
                }
                ("list-for-product-and-sku", Some(opt)) => {
                    call_result = self
                        ._license_assignments_list_for_product_and_sku(opt, dry_run, &mut err)
                        .await;
                }
                ("patch", Some(opt)) => {
                    call_result = self
                        ._license_assignments_patch(opt, dry_run, &mut err)
                        .await;
                }
                ("update", Some(opt)) => {
                    call_result = self
                        ._license_assignments_update(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "license-assignments".to_string(),
                    ));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "licensing1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/licensing1", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::Licensing::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
        ...
        ("license-assignments", "methods: 'delete', 'get', 'insert', 'list-for-product', 'list-for-product-and-sku', 'patch' and 'update'", vec![
            ("delete",
                  ...
                  vec![
                    ...
                    (Some(r##"user-id"##),
                     None,
                     Some(r##"The user's current primary email address. If the user's email address changes, use the new email address in your API requests. Since a `userId` is subject to change, do not use a `userId` value as a key for persistent data. This key could break if the current user's email address changes. If the `userId` is suspended, the license status changes."##),
                     Some(true),
                     Some(false)),
                    (Some(r##"v"##),
                     Some(r##"p"##),
