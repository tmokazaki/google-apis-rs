...
use google_safebrowsing4::{api, yup_oauth2, Error};
...
{
    async fn _encoded_full_hashes_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .encoded_full_hashes()
            .get(opt.value_of("encoded-request").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _full_hashes_find(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "api-client.client-version" => Some((
                ...
                "threat-info.platform-types" => Some((
                ...
                "threat-info.threat-entry-types" => Some((
                    "threatInfo.threatEntryTypes",
                    JsonTypeInfo {
                        jtype: JsonType::String,
                        ctype: ComplexType::Vec,
                    },
                )),
                "threat-info.threat-types" => Some((
        ...
        let mut request: api::GoogleSecuritySafebrowsingV4FindFullHashesRequest =
    ...
    async fn _threat_hits_create(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "client-info.client-version" => Some((
        ...
        let mut request: api::GoogleSecuritySafebrowsingV4ThreatHit =
            serde_json::value::from_value(object).unwrap();
    ...
    async fn _threat_list_updates_fetch(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["client", "client-id", "client-version"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::GoogleSecuritySafebrowsingV4FetchThreatListUpdatesRequest =
            serde_json::value::from_value(object).unwrap();
    ...
    async fn _threat_matches_find(
        ...
        let mut request: api::GoogleSecuritySafebrowsingV4FindThreatMatchesRequest =
            serde_json::value::from_value(object).unwrap();
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("encoded-full-hashes", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._encoded_full_hashes_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "encoded-full-hashes".to_string(),
                    ));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("encoded-updates", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._encoded_updates_get(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("encoded-updates".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("full-hashes", Some(opt)) => match opt.subcommand() {
                ("find", Some(opt)) => {
                    call_result = self._full_hashes_find(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("full-hashes".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("threat-hits", Some(opt)) => match opt.subcommand() {
                ("create", Some(opt)) => {
                    call_result = self._threat_hits_create(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("threat-hits".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("threat-list-updates", Some(opt)) => match opt.subcommand() {
                ("fetch", Some(opt)) => {
                    call_result = self
                        ._threat_list_updates_fetch(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues.push(CLIError::MissingMethodError(
                        "threat-list-updates".to_string(),
                    ));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ...
            ("threat-matches", Some(opt)) => match opt.subcommand() {
                ("find", Some(opt)) => {
                    call_result = self._threat_matches_find(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("threat-matches".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "safebrowsing4-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/safebrowsing4", config_dir))
        .build()
        .await
        .unwrap();
