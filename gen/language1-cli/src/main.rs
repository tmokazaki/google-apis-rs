...
use google_language1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _documents_analyze_entity_sentiment(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::AnalyzeEntitySentimentRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.documents().analyze_entity_sentiment(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _documents_annotate_text(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "features.classification-model-options.v2-model.content-categories-version" => {
                    Some((
                        "features.classificationModelOptions.v2Model.contentCategoriesVersion",
                        JsonTypeInfo {
                            jtype: JsonType::String,
                            ctype: ComplexType::Pod,
                        },
                ...
                "features.extract-entity-sentiment" => Some((
    ...
    async fn _documents_classify_text(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "classification-model-options.v2-model.content-categories-version" => Some((
        ...
        let mut request: api::ClassifyTextRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.documents().classify_text(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _documents_moderate_text(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        key,
                        &vec!["content", "document", "gcs-content-uri", "language", "type"],
                    );
                    err.issues.push(CLIError::Field(FieldError::Unknown(
                        temp_cursor.to_string(),
                        suggestion,
        ...
        let mut request: api::ModerateTextRequest = serde_json::value::from_value(object).unwrap();
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("documents", Some(opt)) => match opt.subcommand() {
                ...
                ("analyze-entity-sentiment", Some(opt)) => {
                    call_result = self
                        ._documents_analyze_entity_sentiment(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "language1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/language1", config_dir))
        .build()
        .await
        .unwrap();
    ...
    let arg_data = [
        ("documents", "methods: 'analyze-entities', 'analyze-entity-sentiment', 'analyze-sentiment', 'analyze-syntax', 'annotate-text', 'classify-text' and 'moderate-text'", vec![
            ...
            ("analyze-entity-sentiment",
                    Some(r##"Finds entities, similar to AnalyzeEntities in the text and analyzes sentiment associated with each entity and its mentions."##),
                    "Details at http://byron.github.io/google-apis-rs/google_language1_cli/documents_analyze-entity-sentiment",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
            ...
            ("analyze-sentiment",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_language1_cli/documents_analyze-sentiment",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
            ...
            ("analyze-syntax",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_language1_cli/documents_analyze-syntax",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
            ...
            ("annotate-text",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_language1_cli/documents_annotate-text",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
            ...
            ("classify-text",
                    Some(r##"Classifies a document into categories."##),
                    "Details at http://byron.github.io/google-apis-rs/google_language1_cli/documents_classify-text",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
