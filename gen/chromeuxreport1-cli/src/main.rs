...
use google_chromeuxreport1::{api, yup_oauth2, Error};
...
{
    async fn _records_query_history_record(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut request: api::QueryHistoryRequest = serde_json::value::from_value(object).unwrap();
        let mut call = self.hub.records().query_history_record(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _records_query_record(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                "effective-connection-type" => Some((
        ...
        let mut call = self.hub.records().query_record(request);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("records", Some(opt)) => match opt.subcommand() {
                ("query-history-record", Some(opt)) => {
                    call_result = self
                        ._records_query_history_record(opt, dry_run, &mut err)
                        .await;
                }
                ("query-record", Some(opt)) => {
                    call_result = self._records_query_record(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("records".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "chromeuxreport1-secret.json",
        ...
        let engine = Engine {
            opt,
            hub: api::ChromeUXReport::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("records", "methods: 'query-history-record' and 'query-record'", vec![
            ("query-history-record",
                    Some(r##"Queries the Chrome User Experience Report for a timeseries `history record` for a given site. Returns a `history record` that contains one or more `metric timeseries` corresponding to performance data about the requested site."##),
                    "Details at http://byron.github.io/google-apis-rs/google_chromeuxreport1_cli/records_query-history-record",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
            ...
            ("query-record",
                    Some(r##"Queries the Chrome User Experience for a single `record` for a given site. Returns a `record` that contains one or more `metrics` corresponding to performance data about the requested site."##),
                    "Details at http://byron.github.io/google-apis-rs/google_chromeuxreport1_cli/records_query-record",
                  vec![
                    (Some(r##"kv"##),
                     Some(r##"r"##),
                     Some(r##"Set various fields of the request structure, matching the key=value form"##),
