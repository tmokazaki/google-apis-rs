...
use google_mybusinesslodging1::{api, yup_oauth2, Error};
...
{
    async fn _locations_get_lodging(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .locations()
            .get_lodging(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _locations_lodging_get_google_updated(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .locations()
            .lodging_get_google_updated(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _locations_update_lodging(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::Lodging = serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .locations()
            .update_lodging(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
        ...
        match self.opt.subcommand() {
            ("locations", Some(opt)) => match opt.subcommand() {
                ...
                ("lodging-get-google-updated", Some(opt)) => {
                    call_result = self
                        ._locations_lodging_get_google_updated(opt, dry_run, &mut err)
                        .await;
                }
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "mybusinesslodging1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/mybusinesslodging1", config_dir))
        .build()
        .await
        .unwrap();
