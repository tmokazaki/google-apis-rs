...
use google_groupsmigration1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _archive_insert(
        ...
        let mut call = self
            .hub
            .archive()
            .insert(opt.value_of("group-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("archive", Some(opt)) => match opt.subcommand() {
                ("insert", Some(opt)) => {
                    call_result = self._archive_insert(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("archive".to_string()));
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "groupsmigration1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/groupsmigration1", config_dir))
        .build()
        .await
        .unwrap();
