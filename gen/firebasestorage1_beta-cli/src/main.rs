...
use google_firebasestorage1_beta::{api, yup_oauth2, Error};
...
{
    async fn _projects_buckets_add_firebase(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        ...
        let mut call = self
            .hub
            .projects()
            .buckets_add_firebase(request, opt.value_of("bucket").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_buckets_get(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .buckets_get(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_buckets_list(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .buckets_list(opt.value_of("parent").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_buckets_remove_firebase(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::RemoveFirebaseRequest =
            serde_json::value::from_value(object).unwrap();
        let mut call = self
            .hub
            .projects()
            .buckets_remove_firebase(request, opt.value_of("bucket").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("projects", Some(opt)) => match opt.subcommand() {
                ("buckets-add-firebase", Some(opt)) => {
                    call_result = self
                        ._projects_buckets_add_firebase(opt, dry_run, &mut err)
                        .await;
                }
                ("buckets-get", Some(opt)) => {
                    call_result = self._projects_buckets_get(opt, dry_run, &mut err).await;
                }
                ("buckets-list", Some(opt)) => {
                    call_result = self._projects_buckets_list(opt, dry_run, &mut err).await;
                }
                ("buckets-remove-firebase", Some(opt)) => {
                    call_result = self
                        ._projects_buckets_remove_firebase(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "firebasestorage1-beta-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/firebasestorage1-beta", config_dir))
        .build()
        .await
        .unwrap();
        ...
        let engine = Engine {
            opt,
            hub: api::Firebasestorage::new(client, auth),
            gp: vec![
                "$-xgafv",
                "access-token",
                "alt",
...
async fn main() {
    ...
    let arg_data = [
        ("projects", "methods: 'buckets-add-firebase', 'buckets-get', 'buckets-list' and 'buckets-remove-firebase'", vec![
            ("buckets-add-firebase",
                    Some(r##"Links a Google Cloud Storage bucket to a Firebase project."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasestorage1_beta_cli/projects_buckets-add-firebase",
                  vec![
                    (Some(r##"bucket"##),
                     None,
            ...
            ("buckets-get",
                    Some(r##"Gets a single linked storage bucket."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasestorage1_beta_cli/projects_buckets-get",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"Required. Resource name of the bucket, mirrors the ID of the underlying Google Cloud Storage bucket, `projects/{project_id_or_number}/buckets/{bucket_id}`."##),
            ...
            ("buckets-list",
                    Some(r##"Lists the linked storage buckets for a project."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasestorage1_beta_cli/projects_buckets-list",
                  vec![
                    (Some(r##"parent"##),
                     None,
            ...
            ("buckets-remove-firebase",
                    Some(r##"Unlinks a linked Google Cloud Storage bucket from a Firebase project."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasestorage1_beta_cli/projects_buckets-remove-firebase",
                  vec![
                    (Some(r##"bucket"##),
                     None,
                     Some(r##"Required. Resource name of the bucket, mirrors the ID of the underlying Google Cloud Storage bucket, `projects/{project_id_or_number}/buckets/{bucket_id}`."##),
