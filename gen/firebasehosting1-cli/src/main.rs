...
use google_firebasehosting1::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    ...
    async fn _projects_sites_custom_domains_operations_cancel(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .sites_custom_domains_operations_cancel(request, opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _projects_sites_custom_domains_operations_delete(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut call = self
            .hub
            .projects()
            .sites_custom_domains_operations_delete(opt.value_of("name").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ...
            ("projects", Some(opt)) => match opt.subcommand() {
                ("sites-custom-domains-operations-cancel", Some(opt)) => {
                    call_result = self
                        ._projects_sites_custom_domains_operations_cancel(opt, dry_run, &mut err)
                        .await;
                }
                ("sites-custom-domains-operations-delete", Some(opt)) => {
                    call_result = self
                        ._projects_sites_custom_domains_operations_delete(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "firebasehosting1-secret.json",
        ...
        .persist_tokens_to_disk(format!("{}/firebasehosting1", config_dir))
        .build()
        .await
        .unwrap();
...
async fn main() {
    ...
    let arg_data = [
        ("operations", "methods: 'cancel', 'delete' and 'list'", vec![
            ("cancel",
                    Some(r##"Starts asynchronous cancellation on a long-running operation. The server makes a best effort to cancel the operation, but success is not guaranteed. If the server doesn't support this method, it returns `google.rpc.Code.UNIMPLEMENTED`. Clients can use Operations.GetOperation or other methods to check whether the cancellation succeeded or whether the operation completed despite cancellation. On successful cancellation, the operation is not deleted; instead, it becomes an operation with an Operation.error value with a google.rpc.Status.code of 1, corresponding to `Code.CANCELLED`."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasehosting1_cli/operations_cancel",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("delete",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_firebasehosting1_cli/operations_delete",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("list",
                    ...
                    "Details at http://byron.github.io/google-apis-rs/google_firebasehosting1_cli/operations_list",
                  vec![
                    (Some(r##"name"##),
                     None,
            ...
            ("projects", "methods: 'sites-custom-domains-operations-cancel' and 'sites-custom-domains-operations-delete'", vec![
            ("sites-custom-domains-operations-cancel",
                    Some(r##"CancelOperation is a part of the google.longrunning.Operations interface, but is not implemented for CustomDomain resources."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasehosting1_cli/projects_sites-custom-domains-operations-cancel",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"The name of the operation resource to be cancelled."##),
            ...
            ("sites-custom-domains-operations-delete",
                    Some(r##"DeleteOperation is a part of the google.longrunning.Operations interface, but is not implemented for CustomDomain resources."##),
                    "Details at http://byron.github.io/google-apis-rs/google_firebasehosting1_cli/projects_sites-custom-domains-operations-delete",
                  vec![
                    (Some(r##"name"##),
                     None,
                     Some(r##"The name of the operation resource to be deleted."##),
    ...
    let mut app = App::new("firebasehosting1")
           ...
           .after_help("All documentation details can be found at http://byron.github.io/google-apis-rs/google_firebasehosting1_cli")
