...
use google_doubleclicksearch2::{api, yup_oauth2, Error};
    ...
    C: apis_common::Connector,
{
    async fn _conversion_get(
            ...
            .iter()
        {
            ...
            match key {
                ...
                "criterion-id" => {
                    call = call.criterion_id(
                        value
                            .map(|v| arg_from_str(v, err, "criterion-id", "int64"))
                            .unwrap_or(-0),
                    );
                }
                "campaign-id" => {
                    call = call.campaign_id(
                        value
                            .map(|v| arg_from_str(v, err, "campaign-id", "int64"))
                            .unwrap_or(-0),
                    );
                }
                "ad-id" => {
                    call = call.ad_id(
                        value
                            .map(|v| arg_from_str(v, err, "ad-id", "int64"))
                            .unwrap_or(-0),
                    );
                }
                "ad-group-id" => {
                    call = call.ad_group_id(
                        value
                            .map(|v| arg_from_str(v, err, "ad-group-id", "int64"))
                            .unwrap_or(-0),
                    );
                }
                _ => {
    ...
    async fn _conversion_get_by_customer_id(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
            ...
            .iter()
        {
            ...
            match key {
                "engine-account-id" => {
                    call = call.engine_account_id(
                        value
                            .map(|v| arg_from_str(v, err, "engine-account-id", "int64"))
                            .unwrap_or(-0),
                    );
                }
                "criterion-id" => {
                ...
                "agency-id" => {
                    call = call.agency_id(
                        value
                            .map(|v| arg_from_str(v, err, "agency-id", "int64"))
                            .unwrap_or(-0),
                    );
                }
                "advertiser-id" => {
    ...
    async fn _conversion_insert(
        ...
        let mut request: api::ConversionList = serde_json::value::from_value(object).unwrap();
    ...
    async fn _conversion_update_availability(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
        ...
        let mut request: api::UpdateAvailabilityRequest =
    ...
    async fn _reports_generate(
            ...
            .iter()
        {
            ...
            let type_info: Option<(&'static str, JsonTypeInfo)> = match &temp_cursor.to_string()[..]
            {
                ...
                "report-scope.advertiser-id" => Some((
                ...
                "report-scope.engine-account-id" => Some((
                ...
                "time-range.changed-attributes-since-timestamp" => Some((
                ...
                "time-range.changed-metrics-since-timestamp" => Some((
                ...
                _ => {
                    let suggestion = FieldCursor::did_you_mean(
                        ...
                        &vec![
                            ...
                            "agency-id",
                            "campaign-id",
                            "changed-attributes-since-timestamp",
                            "changed-metrics-since-timestamp",
                            "download-format",
                            "end-date",
                            "engine-account-id",
    ...
    async fn _reports_get(
        ...
        let mut call = self
            .hub
            .reports()
            .get(opt.value_of("report-id").unwrap_or(""));
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _reports_get_file(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ) -> Result<(), DoitError> {
        let report_fragment: i32 = arg_from_str(
        ...
        let mut call = self
            .hub
            .reports()
            .get_file(opt.value_of("report-id").unwrap_or(""), report_fragment);
        for parg in opt
            .values_of("v")
            .map(|i| i.collect())
            .unwrap_or(Vec::new())
    ...
    async fn _reports_get_id_mapping_file(
        &self,
        opt: &ArgMatches<'n>,
        dry_run: bool,
        err: &mut InvalidOptionsError,
    ...
    async fn _doit(
        ...
        match self.opt.subcommand() {
            ("conversion", Some(opt)) => match opt.subcommand() {
                ("get", Some(opt)) => {
                    call_result = self._conversion_get(opt, dry_run, &mut err).await;
                }
                ("get-by-customer-id", Some(opt)) => {
                    call_result = self
                        ._conversion_get_by_customer_id(opt, dry_run, &mut err)
                        .await;
                }
                ("insert", Some(opt)) => {
                ...
                ("update-availability", Some(opt)) => {
                    call_result = self
                        ._conversion_update_availability(opt, dry_run, &mut err)
                        .await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("conversion".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("reports", Some(opt)) => match opt.subcommand() {
                ("generate", Some(opt)) => {
                ...
                ("get-id-mapping-file", Some(opt)) => {
                    call_result = self
                        ._reports_get_id_mapping_file(opt, dry_run, &mut err)
                        .await;
                }
                ("request", Some(opt)) => {
                ...
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("reports".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            ("saved-columns", Some(opt)) => match opt.subcommand() {
                ("list", Some(opt)) => {
                    call_result = self._saved_columns_list(opt, dry_run, &mut err).await;
                }
                _ => {
                    err.issues
                        .push(CLIError::MissingMethodError("saved-columns".to_string()));
                    writeln!(std::io::stderr(), "{}\n", opt.usage()).ok();
                }
            },
            _ => {
    ...
    async fn new(opt: ArgMatches<'n>, connector: C) -> Result<Engine<'n, C>, InvalidOptionsError> {
        let (config_dir, secret) = {
            ...
            match common::application_secret_from_directory(&config_dir, "doubleclicksearch2-secret.json",
